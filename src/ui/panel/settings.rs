...
pub fn view(settings: &AppSettings) -> iced::Element<'_, crate::ui::Message> {
    ...
    let log_body: iced::Element<_> = row![
        text("Log Level").size(13).color(theme::TEXT_SECONDARY).width(Length::Fixed(140.0)),
    ...
    let timeout_body: iced::Element<_> = row![
        text("Timeout (s)").size(13).color(theme::TEXT_SECONDARY).width(Length::Fixed(140.0)),
    ...
    let retry_body: iced::Element<_> = row![
        text("Retry Count").size(13).color(theme::TEXT_SECONDARY).width(Length::Fixed(140.0)),
    ...
    let delay_body: iced::Element<_> = row![
        text("Retry Delay (ms)").size(13).color(theme::TEXT_SECONDARY).width(Length::Fixed(140.0)),
    ...
    container(
        column![
            ...
            card_section("Connection", column![log_body, Space::new().height(16.0), timeout_body].into()),
            Space::new().height(12.0),
            card_section("Retry Policy", column![retry_body, Space::new().height(16.0), delay_body].into()),
